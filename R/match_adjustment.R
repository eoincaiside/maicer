...
match_adjustment <- function(matching_object, outcomes = NULL, arm_identifier = NULL)
{
  ...
  if (!is.null(outcomes) & !is.null(arm_identifier))
  {
    ...
    if(length(arm_identifier) != nrow(match_adjustment_object$matching_set)  )
    {
      stop("Number of patient outcomes not equal to number of patients.")
    }
...
{
  plot(plot_weights(x$rescaled))
  # plot_verify(object$verification)
}

#' @export
summary.maic.match_adjustment <- function(object, ...)
{
